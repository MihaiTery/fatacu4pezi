...
**The Favicon-Mark Texture.** A repeating tile of the client's new favicon mark (`public/favicon.svg` — four small abstract squiggle shapes arranged into a "4," for "4pezi," inside a cream badge circle; the badge circle itself is dropped for the texture, only the line-art "4" is used) is baked into two CSS custom properties (`--pattern-mark-ink`, `--pattern-mark-cream`) and exposed as three utilities in `global.css`: `bg-mark` / `bg-mark-invert` (the texture alone, ink- or cream-tinted) and `alt-band` (paper + the ink-tinted texture, bundled). It appears only on paper alt-bands and the green header/footer (`SiteFooter.astro` and the green nav row + mobile menu panel in `SiteHeader.astro` all carry `bg-mark-invert`) — never on cream, which stays the one flat, quiet ground. Each tile holds two copies of the mark offset by half a tile on both axes, so the repeat reads as a staggered/brick grid, not a plain square one; fill-opacity (0.09 ink / 0.11 cream) and the 200px render size were both raised from an initial quieter pass, on an explicit client request ("mai evident, mai des si intercalat" — more visible, denser, interleaved). If a future pass needs it quieter again, dial opacity and tile size back down together rather than picking one — the two moves were paired deliberately. The mark's four squiggles read as paw-adjacent at a glance — this is a deliberate, scoped exception to the standing "no paw prints" ban, the same way the Hero Mark's dog face is a scoped exception to the hand-authored-icon system: the client supplied this exact artwork as the site's new favicon, so it is used as given, not redrawn to dodge the ban, and it is not license to add paw imagery anywhere else in the system.
...
### Don't:
- **Don't** use hearts, blobs, or literal transit/bus/tram iconography anywhere in the system. Paw prints stay off the standing ban too — the hero's dog mark (the inlined Noto Emoji dog face) and the client-supplied favicon mark (its four squiggles, used only as the sitewide background texture — see Colors → Named Rules → The Favicon-Mark Texture) are each a specific, client-directed exception for that one mark, not a reopening of decorative animal or paw motifs generally, and not license to import more paw/emoji imagery elsewhere in the system.
