...
- Hand-authored 24×24 stroke icon set (1.75px stroke) for UI icons — no icon library, no emoji; Hero Chase's cat/dog/heart emoji and favicon-as-chaser are a separate, scoped easter-egg register, not part of that UI icon set.
...
- One consistent motion grammar sitewide (quiet fade-and-rise on scroll, a hairline-to-shadow lift on interactive cards, Hero Chase's maze loop) rather than scattered, mismatched effects — see Components → Motion.
...
- **Sky Blue** (#c4e0f5): the lighter-commitment button/accent color — the `accent` button variant (a 2px blue-bordered pill, e.g. the header's "Redirecționează 3,5%"), the "in_tratament" status pill fill, one of two faint background rings in Hero Chase. Elevated from chip-only to button scale in this pass, alongside green, on explicit client direction ("butoane, accente și detalii" in blue and green).
...
- **Brand Ink** (#3e1d0d): all body text, headings, line art (icons, route-line dashes, Hero Chase's maze path), default border tone (`ink/10`, `ink/15`, `ink/25` opacity steps) on cream surfaces, and — since this pass — the text/icon/border color on the pink header and footer too. Ink is now the system's only "on light surface" text color; there is no cream-surface/dark-surface text split left to track.
- **Soft Ink** (#6b4a35): secondary/supporting text — card descriptions, meta rows, body copy that isn't the primary read.

### Secondary
- **Blush Chip Pink** (#f2cfdc): at chip scale (not surface scale), the "rezervat" status pill fill and one of two faint background rings in Hero Chase — the same hex as the header/footer surface, used two registers up in size, which is fine since it's one brand hue doing two different jobs, not two competing pinks.
...
### Hero Chase (signature component, was Hero Mark)
The homepage's second signature device, filling the hero's right column at `lg` and above (hidden below `lg` — the homepage stays short on mobile rather than adding scroll for decoration). Replaces the earlier literal "dog runs home" illustration (a dog traveling a rising dashed path to a small house) with a playful maze-chase easter egg, on explicit client request: a cat and a dog (native emoji, 🐱/🐶) chase each other along the same dashed route-line grammar the rest of the system uses for status/journey/rhythm devices — the maze corridor *is* the brand's route line, not a new decorative device — while the site's own favicon mark trails them Pac-Man style, closing the gap over the loop. When it catches up, a heart emoji (❤️) pops in at the meeting point and the whole loop resets. Two faint tonal rings (sky blue, blush pink) carry over from the old Hero Mark for depth. Built in `HeroChase.astro`, replacing `HeroMark.astro` (removed, along with the inlined Noto Emoji dog-face asset it used — see git history for that prior version and its OFL-1.1 attribution).

This is a deliberate, scoped exception to **two** standing rules at once: the no-hearts ban and the hand-authored-icon/no-emoji system (see Do's and Don'ts). Unlike the old Hero Mark's single inlined-emoji exception (the dog face, vector paths baked in), this one uses native emoji glyphs directly (`<text>` in SVG) for cat, dog, and heart alike, plus the site's real `favicon.svg` as the chaser — chosen for a self-contained, silly, easter-egg register that's explicitly not asking to be taken as seriously as the rest of the system's restrained, competence-first visual language. It is not license to add more hearts, paw prints, or emoji elsewhere in the system; it stays confined to this one hero device. Purely graphic (`aria-hidden="true"`) — nothing here is unique information, so nothing is lost for assistive tech, and `prefers-reduced-motion` freezes it on a static, heart-free arrangement (see Motion).
...
- **Hero Chase entrance + loop:** on page load (not scroll-gated, since it's above the fold), the mark fades/scales in and its maze route-line draws itself (`stroke-dashoffset`), same technique as the old Hero Mark. 1.6s in, a single shared 9-second loop begins: cat and dog run the maze path (SVG `<animateMotion>`/`<mpath>`, not CSS `offset-path`, for the same coordinate-space reason as the old Hero Mark), the dog phased a fixed step behind the cat so the two always read as chasing each other; the favicon starts further back and covers the same ground faster, so it visibly closes the gap over the loop. All three converge on the same point in the maze by design (hand-tuned keyPoints/keyTimes on one shared timeline, verified with `getPointAtLength()` rather than eyeballed) — a heart (CSS keyframes, timed to the same 9s/1.6s cycle) pops in at that exact point, everything holds a beat, then fades out together and the loop restarts. Each traveler pulses gently while moving (`<animateTransform type="scale">`, `additive="sum"`, on an inner `<g>` with no static transform of its own, so it never fights the outer `<g>`'s static positioning) — the same pattern the old Hero Mark's dog used.
- **Reduced motion:** every CSS animation on the site — reveal, card-lift's transition (not its final state), RouteJourney, Hero Chase's entrance and heart-pop — is neutralized by one global rule (`prefers-reduced-motion: reduce` sets all `animation-duration`/`transition-duration` to near-zero) plus per-component fallback rules that render the *finished* state immediately. SMIL (the chase's `<animateMotion>`/`<animate>`/`<animateTransform>`) doesn't obey that CSS rule at all, so Hero Chase checks `matchMedia` in a small inline script, same as the old Hero Mark. `pauseAnimations()` alone isn't sufficient — a paused SMIL animation still owns its target attribute and silently overrides any `setAttribute()` on the same attribute — so the script removes the `<animateMotion>`/`<animate>`/`<animateTransform>` elements outright (releasing the attribute back) before setting each traveler's resting position and opacity directly, on a static point of the maze path. The heart stays hidden in reduced motion (no catch narrative without motion) via the same global near-zero-duration rule collapsing its CSS keyframes to their final, hidden state.
...
- **Do** build any new UI icon in the existing 24×24, 1.75px-stroke, round-cap hand-authored system rather than pulling in an icon library or emoji; Hero Chase's cat/dog/heart emoji and the favicon-as-chaser are a separate, scoped easter-egg register (see Components → Hero Chase) — not a precedent for adding more emoji or illustrative icons elsewhere in the system.
...
### Don't:
- **Don't** use hearts, blobs, or literal transit/bus/tram iconography anywhere in the system. Paw prints stay off the standing ban too. Three specific, named exceptions exist and are not a reopening of decorative animal, paw, or heart motifs generally, nor license to import more emoji/heart imagery elsewhere in the system: the client-supplied favicon mark (its four squiggles, used both as the sitewide background texture — see Colors → Named Rules → The Favicon-Mark Texture — and, as the artwork itself, as the chaser in Hero Chase); and Hero Chase's cat/dog/heart emoji, confined to that one hero device (see Components → Hero Chase).
